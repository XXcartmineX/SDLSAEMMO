...
b2Vec2 RPGSAEMMOApp::gravity = b2Vec2_zero;
fVector2 RPGSAEMMOApp::camera = fVector2::zero;
b2World* RPGSAEMMOApp::world = nullptr;
...
{
	scalingFactor = scale;
	getWorld()->ShiftOrigin(b2Vec2(-sizeX / 2, -sizeY / 2));
	...
	camera = fVector2(sizeX / 2, sizeY / 2);
...
void RPGSAEMMOApp::AddToPhysicsQueue(std::function<void()> f)
{
...
int RPGSAEMMOApp::Physics(void * ptr)
{
	while (!appIsClosing)
	{
		world->Step(physicsFrameLock, velocityIterations, positionIterations);
		if (physicsQueue.size() <= 0) {
			SDL_Delay(10);
			continue;
		}
		SDL_LockMutex(physicsMutex);
		std::function<void()> f = physicsQueue.front();
		physicsQueue.pop();
		SDL_UnlockMutex(physicsMutex);
		f();
	}
